...
def safe_str(val: Any) -> str:
    if val is None:
        return "-"
    # Convert to string and replace any non-latin1 characters to prevent FPDF encoding crashes
...
def _generate_pdf(case: dict, analysis: dict) -> bytes:
    ...
    info_rows = [
        ("Case ID", safe_str(case.get("case_id", "-"))),
        ("Applicant", safe_str(case.get("applicant_name", "-"))),
        ("Loan Type", safe_str(case.get("loan_type", "-"))),
        ("Loan Amount", f"Rs. {case.get('loan_amount', 0):,.0f}"),
        ("Branch", safe_str(case.get("branch", "-"))),
        ("Analysis Date", safe_str(analysis.get("analyzed_at", datetime.utcnow().isoformat())[:19])),
    ...
    pdf.cell(0, 12, f"  VERDICT: {verdict}  |  Confidence: {safe_str(analysis.get('confidence', '-'))}  |  Overall Score: {analysis.get('overall_score', 0):.1f}%", ln=True, fill=True)
    pdf.set_text_color(0, 0, 0)
    pdf.ln(5)
    ...
    if findings:
        ...
        for finding in findings[:15]:  # Max 15 in PDF
            ...
            f_type = safe_str(finding.get("type", "FINDING"))
    ...
    for i, doc in enumerate(analysis.get("document_reports", []), 1):
        filename = safe_str(doc.get('filename', 'Unknown'))
        doc_type = safe_str(doc.get('type', 'unknown').upper())
        pdf.set_font("Helvetica", "B", 10)
        pdf.cell(0, 7, f"  {i}. {filename} [{doc_type}]", ln=True)
        pdf.set_font("Helvetica", "", 9)
        ...
        if doc.get("pdf_forensics"):
            ...
            if meta.get("producer"):
                pdf.cell(0, 6, f"     PDF Producer: {safe_str(meta['producer'])}", ln=True)
            if meta.get("creation_date"):
                pdf.cell(0, 6, f"     Created: {safe_str(meta['creation_date'])[:19]}", ln=True)
            if meta.get("mod_date"):
                pdf.cell(0, 6, f"     Modified: {safe_str(meta['mod_date'])[:19]}", ln=True)
        pdf.ln(2)
